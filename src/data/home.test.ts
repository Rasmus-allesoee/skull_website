...
describe("Home page model", () => {
  it("covers every published physical specimen exactly once across bounded hero states", () => {
    ...
    expect(
      model.heroStates.every((state) => state.specimens.length === 6),
    ).toBe(true);
    expect(new Set(visibleIds).size).toBe(visibleIds.length);
    expect(visibleIds.slice().sort()).toEqual(publishedIds.slice().sort());
    expect(
      model.heroStates
