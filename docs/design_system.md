...
- **Home destination hub:** one two-column-to-single-column grid containing Species, Map, Measurements, Preparation, and a non-linking `Coming soon` Comparison preview. Whole-card links have permanent text actions; the future card is equally designed but unmistakably unavailable. Static previews use the owner-supplied, metadata-stripped thumbnail derivatives declared in `content/home/home-media.json`; they do not embed destination controls or a second map/search experience.
...
The preparation guide uses a five-stage image workflow with real phase/method links, short completion outcomes and an optional-whitening branch. Defleshing methods are numbered (`1.`–`5.`), and maceration troubleshooting is a smaller, indented `1.1.` disclosure beneath Water maceration. It opens four collapsed case rows; the dark-bone and adipocere rows show square owner-photo thumbnails on the right, with the thumbnail links kept beside (not inside) the native summaries for valid interactive semantics. A bounded sticky strip opens a native left-hand contents drawer; Escape/backdrop/Close restore focus, destination links focus their heading, and a native disclosure provides the static fallback. Reading position is a navigation cue, not a progress-completion claim.
