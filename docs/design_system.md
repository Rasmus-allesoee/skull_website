...
- **Home specimen field:** a compact, border-light museum field beside the primary introduction, using six transparent published skulls per arrangement at deliberate foreground/mid/background depths. Images may overlap but retain complete hit zones, exact specimen links, visible focus, and compact identity disclosure. Desktop pointer movement produces only small bounded depth offsets; first touch selects and reveals identity, while a second deliberate activation opens the exact record. The static first arrangement, all destination cards, and counts remain complete without JavaScript.
- **Home destination hub:** one two-column-to-single-column grid containing Species, Map, Measurements, Preparation, and a non-linking `Coming soon` Comparison preview. Whole-card links have permanent text actions; the future card is equally designed but unmistakably unavailable. Static previews reuse canonical specimen media, reviewed coordinates, registered measurement geometry, the processed owner image, and the existing calibrated-comparison records rather than embedding destination controls.
