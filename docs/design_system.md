...
**Status:** Phase 2.1 representative system implemented; owner re-review pending
...
- 1440 px and wider specimen displays.
...
- Gallery presentation requests a quality-90 responsive image with viewport-accurate sizing; never enlarge a small responsive derivative until it becomes visibly soft or blocky.
...
- **Gallery:** complete main figure, labelled thumbnails, previous/next, direct selection, focused arrow/Home/End keys, swipe, desktop double-click, touch double-tap, concise interaction hints, view label, and completeness state. At the 1440 × 900 reference viewport, the complete main image, controls, and six thumbnails fit without page scrolling.
- **Responsive view rail:** right of the image on desktop and mobile landscape, independently scrollable when future sets exceed its height; below the image in mobile portrait.
- **Inspection dialog:** symmetric full-viewport native modal; original high-resolution asset; initial/return focus; Escape; wheel/trackpad and pinch zoom; constrained mouse/touch drag; slider, buttons, keyboard shortcuts, view navigation, and reduced-motion behavior. Browser zoom modifiers are not intercepted.
...
- **Measurement panel:** primary definition list, progressively disclosed additional values, canonical units, measurement-guide dialog, and honest proportional length reference.
- **Collection record:** owner, source, date, location/precision, sex, age, and condition; reference dialogs for age and the five-level condition scale; pathology, trauma, teeth set, and retained skeleton in one additional-data disclosure.
- **Preparation timeline:** semantic ordered list; unknown dates/durations do not break order.
- **Guide dialog:** native modal semantics, labelled close control, scrollable table, mobile row cards, Escape, and focus restoration.
...
- measurement panel, proportional size reference, and measurement-guide direction;
...
Initial implementation evidence on 2026-08-13 led to the owner's detailed Phase 2.1 review. The resulting 2026-08-14 design replaces the oversized single-column gallery with the responsive view rail, upgrades responsive image quality and the true-resolution inspector, adds working touch gestures, moves measurements first, removes premature profile/rights/diagram sections, and adds the record guidance/progressive-disclosure system. Automated and manual evidence is recorded in `project_status.md`; the gate remains open for final owner approval before Phase 3.
