...
- **Standalone comparison workbench:** a compact 18–21rem selected-subject rail and dominant calibrated field at wide widths, an internally scrolling subject strip at narrower widths, and a semantic all-subject measurement table that becomes one card per row on phones. Up to five numbered/shape-marked subjects may own ten total lateral, frontal, dorsal, ventral, or mandible-dorsal layers. The field applies one shared physical scale and one camera zoom/pan; only layers translate or change stacking. Subject opacity stays inside a secondary disclosure. Toolbar controls favor native selects, range inputs, compact text buttons, tooltips/titles, and one low-priority overflow menu rather than permanent control rows.
- **Comparison manipulation:** alpha-derived silhouettes receive pointer drag while keyboard focus remains generous. Selection adds a non-color outline and compact layer toolbar; Arrow/Shift+Arrow moves, explicit buttons change stacking/remove, empty-field drag pans, modified wheel or two-finger touch zooms/pans, and `Fit all`/`Reset layout` always recover the composition. Field zoom is labelled and changes every skull together; it never implies altered recorded measurements or calibrated monitor life-size output.
