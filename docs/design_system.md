...
- Zoom and the standalone comparison field use the original validated public
  WebP rather than enlarging a small responsive optimizer variant; retained
  `Photo:`/rights context stays available wherever the image is presented.
...
- **Home specimen field:** a compact, border-light museum field beside the primary introduction, using ten transparent published skulls per arrangement at deliberate foreground/mid/background depths. One normalized depth value derives each placement's scale, sharpness, brightness, parallax strength, shadow, and stacking so the cues remain coherent. Lateral views remain dominant while reviewed oblique, frontal, dorsal, ventral, and mandible views add visual breadth. Images may overlap but retain alpha-derived silhouette hit areas, exact specimen links, visible focus, and compact identity disclosure. Pointer and touch movement changes only the visual image inside the fixed link target; first touch selects and reveals identity and, on small touch viewports, gently repels nearby visual layers from the selected specimen, while a second deliberate activation opens the exact record. The repulsion is distance-weighted, safe-area-clamped, and omitted under reduced motion. The field-level identity card prefers an adjacent placement and clamps inside the field with a safety margin. The static first arrangement, all destination cards, and counts remain complete without JavaScript.
- **Home destination hub:** one two-column-to-single-column grid containing live whole-card links for Species, Map, Compare, Measurements, and Preparation. Every card has a permanent text action. Static previews use the owner-supplied, metadata-stripped thumbnail derivatives declared in `content/home/home-media.json`; they do not embed destination controls or a second map/search/comparison experience.
...
- **Control region:** compact bounded operational surface directly below the short catalog heading. Search/mode lead; wide screens keep the class presets and compact labelled action controls on one row. At medium widths, the class presets become one labelled native select so the action controls stay beside it rather than dropping to a second row. At narrow widths, the search label becomes visually hidden, result mode and class become labelled native selects, Filters and Browse taxonomy become labelled icon controls, sort direction becomes an icon, and active-state chips stay in one horizontally scrollable row. The complete sticky region may occupy at most one quarter of a 390 × 844 viewport so at least three quarters remain available to the published displays. It remains sticky at short heights and uses hover/focus tooltips plus accessible names for icon controls.
...
- **Specimen selector:** current specimen and concise alternative list; preserves taxon context and scroll position during same-taxonomy specimen transitions.
...
- **Scale comparison:** vertically stacked lateral skulls share one mathematically calibrated maximum-length scale. The current specimen stays primary; an adult-human reference is the default comparison. Preserve full morphology, use compiled subject bounds rather than transparent canvas width, flip only in presentation when orientation differs, and label approximate reference values. A selected collection specimen is a semantic exact-record link activated by double-click and keyboard; non-collection references remain non-navigable. A concise selected-record note may appear below the selector; generic scale mechanics are not repeated in public copy.
...
- **Standalone comparison workbench:** a compact 18–21rem selected-subject rail and dominant calibrated field at wide widths, an internally scrolling subject strip at narrower widths, and one semantic all-subject measurement table at every width. On phones the table scrolls horizontally within its own region, keeps Measurement frozen, and stacks each compact `Skull X` header above its English name; wider headers retain their inline name and full metadata. Between 64.01rem and 75rem, the side-by-side subject rail stays in place while the field title and controls use separate rows to keep Arrange fully visible. On phones, More follows Fit all on Arrange's control row, with zoom below. Up to five numbered/shape-marked subjects may own ten total lateral, frontal, dorsal, ventral, or mandible-dorsal layers. The field applies one shared physical scale and one camera zoom/pan; only layers translate or change stacking. Subject opacity stays inside a secondary disclosure. The desktop rail is constrained to the field panel's actual height so five cards do not create a second empty vertical region. Toolbar controls favor native selects, range inputs, compact text buttons, tooltips/titles, and one low-priority overflow menu rather than permanent control rows. Native card reordering supplies an isolated card-sized drag preview instead of allowing the browser to snapshot surrounding rail content. Reordering cards or table columns keeps the Difference pair's positional Skull slots fixed, so the records in those slots become its new directed values.
- **Comparison manipulation:** alpha-derived silhouettes receive pointer drag while keyboard focus remains generous; transparent figure/image wrappers are pointer-transparent so only the alpha path can intercept a pointer. Selection adds a non-color outline and compact layer toolbar; Arrow/Shift+Arrow moves, explicit buttons change stacking/remove, Cmd/Control-modified empty-field drag pans the camera, Ctrl/Command+wheel and the 25–2,000% slider zoom, and `Fit all`/`Reset layout` always recover the composition. Ordinary desktop wheel/trackpad input over the field remains document scrolling rather than field movement; a non-passive native listener suppresses that page scroll during modified zoom. One-finger touch starting on a skull moves it in any direction without page scrolling; a touch starting on empty field space scrolls the page, and two-finger touch navigates the camera. A movable bare 100 mm scale bar grows with field zoom; its display-calibration caveat lives in a collapsed-by-default field guide whose complete text wraps when opened. View labels can be hidden, `Custom` preserves manual positions while centering newly added layers, and outside activation/Escape dismisses transient controls. Field zoom changes every skull together; it never implies altered recorded measurements or calibrated monitor life-size output.
- **Comparison exports:** keep Copy link, a small `Download CSV` disclosure, and a low-priority `More` disclosure in the page heading. The CSV disclosure names long/tidy and wide formats; More offers field PNG with pure black or grid background. Native menus close after an action or outside activation, and the existing live status reports completion or failure. A PNG contains the visible skull composition, optional view labels and scale bar, without selection outlines or editor controls.
...
- **Measurement preview/detail:** hover and focus expose a compact name tooltip anchored to the active number, preferring the space above it and adapting to diagram/viewport edges. First touch selects and shows a compact semi-transparent dark/brass name control; tapping its name or activating the annotation again opens one compact non-modal name/number/description panel. The preview prefers a visible below-diagram extension and falls back to the same four-way anchor placement or visible viewport bottom without obscuring the board with a full-width row. The panel has no backdrop, stays fixed to the visible zoomed viewport, permits simultaneous diagram inspection, closes with its control or Escape, and restores keyboard focus. Outside dismissal is a primary tap only (movement beyond 10 CSS pixels, pointer cancellation, and multi-touch do not clear selection), and table-origin dismissal returns to the table. There is no remote reset control.
- **Measurement reference table:** semantic three-column table of all 21 definitions below the board. Narrow layouts render readable stacked rows without changing table associations. A number action does not repeat the visible description: it isolates the matching annotation and moves to its preferred existing diagram, using lateral skull for repeated numbers 1 and 2; outside dismissal returns to the originating row so browsing can continue at the selected definition. The table is the complete no-JavaScript and non-visual equivalent, not a filter result.
...
- No general-purpose parallax, continuous floating, cursor decoration, autoplay rotation, or decorative looping. The sole current exception is the owner-approved Home specimen field: its pointer response is small, bounded, user-driven, visually meaningful as depth, and absent under reduced motion. It never moves page layout, changes arrangement while a specimen is focused, or gates identity/navigation.
...
The 2026-09-01 Home milestone replaces that earlier entrance with the static-first interactive specimen field and one compact five-card hub. It retains the six canonical statistics, removes outdated Home class/tree/editorial sections, keeps MapLibre and search route-local, and introduces only one narrowly scoped motion exception with reduced-motion, keyboard, touch, forced-color, no-JavaScript, and image-failure coverage.
