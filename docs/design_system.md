...
- **Comparison manipulation:** alpha-derived silhouettes receive pointer drag while keyboard focus remains generous. Selection adds a non-color outline and compact layer toolbar; Arrow/Shift+Arrow moves, explicit buttons change stacking/remove, empty-field drag and unmodified desktop wheel pan, Ctrl/Command+wheel and the 25–2,000% slider zoom, and `Fit all`/`Reset layout` always recover the composition. One-finger vertical touch leaves document scrolling available, deliberate horizontal touch can move a layer, and two-finger touch navigates the camera. A movable viewport scale bar grows with field zoom, view labels can be hidden, and outside activation/Escape dismisses transient controls. Field zoom changes every skull together; it never implies altered recorded measurements or calibrated monitor life-size output.
