...
- **Taxon card:** a bounded, equal-height lateral-image stage leads a compact facts area. Show common/scientific/Danish names, specimen count, skull length, and skull mass. The metrics come from the largest recorded specimen (or largest specimen still matching the active query/filters) and may differ from the curated default lateral image; label the specimen ID as `Largest recorded` or `Largest matching` when it is useful. Do not repeat a separate confidence/uncertainty line when the scientific label already communicates a genus-level `sp.` record.
- **Specimen card:** use the same bounded image stage and compact two-column fact treatment. Show the immutable specimen ID, common/scientific/Danish names, skull length, skull mass, age, sex, condition, and location/date when space permits. Keep missing values explicit and use dividers to make the dense facts legible; do not make the card tall merely to enlarge the image.
