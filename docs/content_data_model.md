...
Guide MDX uses explicit title, slug, summary, review date, safety-review status, and citations. Preparation guidance must distinguish documented personal/community practice from established safety guidance. Chemical concentrations, animal handling, disease, legal collection rules, and disposal claims require current authoritative review before publication. The owner-authorized `/guides/skull-preparation` implementation now uses `content/guides/skull-preparation.mdx` and the restricted compiler described in [skull_preparation_guide.md](skull_preparation_guide.md). `review_status: reviewed` and `safety_review: source-checked` record editorial source checking, not independent professional certification. Literal Figure/Aside/Details/Disclosure components, optional Details media attributes, internal Subheading blocks, stable explicit heading and disclosure IDs, flat lists, tables, claim-level `[cite:key]` markers, and descriptive media links in the form `[label](asset:asset-id)` compile to typed blocks without executing MDX. Details thumbnails render in a compact left gutter beside (rather than inside) their native summary, outside the case divider lines, so the image control is not nested in the disclosure control. Disclosures can contain non-nested callouts/details but not another disclosure or authored heading. Unresolved/unused/duplicate citations, bad workflow targets, duplicate anchors, malformed tables, undeclared/unused media, and unsupported executable syntax fail the build. Undated guide citations use a null year and render `n.d.`; existing taxon-profile requirements are unchanged.

Preparation media lives in `content/guides/preparation-media.json`: eighteen stable editorial IDs with paths, alt text, captions, credits, rights, and `owner-photograph`/`ai-illustration` provenance. `pnpm media:process:preparation` processes the maintenance source map into full-aspect-ratio sRGB WebPs under `public/media/preparation/` (1000–1600 px longest edge, maximum 750 kB each, no EXIF/GPS/ICC/IPTC/XMP), trying deterministic size/quality fallbacks for large photographic sources. Content/media checks validate public files and declaration/use parity; normal builds never read the ignored `agent_context/preparation_page/` masters. The generated guide is `.generated/preparation-guide-v1.json`. Condition-table media tokens are validated against the same declaration and progressively enhanced from ordinary image links to a native quick-preview dialog. Descriptive media links and Details thumbnails use the same declaration and progressively enhance to a native body-level lightbox with focus restoration; without JavaScript they remain direct WebP links. No collection ID, measurement, or specimen preparation field is changed.
