...
Preparation media lives in `content/guides/preparation-media.json`: eight stable editorial IDs with paths, alt text, captions, credits, rights, and `owner-photograph`/`ai-illustration` provenance. `pnpm media:process:preparation` processes the maintenance source map into full-aspect-ratio sRGB WebPs under `public/media/preparation/` (1000–1600 px longest edge, maximum 750 kB each, no EXIF/GPS/ICC/IPTC/XMP). Content/media checks validate public files and declaration/use parity; normal builds never read the ignored `agent_context/preparation_page/` masters. The generated guide is `.generated/preparation-guide-v1.json`. No collection ID, measurement, or specimen preparation field is changed.
