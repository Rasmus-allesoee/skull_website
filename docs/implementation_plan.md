...
- [x] Complete the separately owner-authorized preparation guide on its existing route and implement owner-feedback v1: beginner-first five-stage workflow, method-specific native disclosures, full practical method coverage, fourteen curated images, six responsive tables, drawer/static navigation, inline citation cards, complete references, and canonical documentation. Current verification is recorded in `project_status.md`; a broader Guides hub remains deferred.
- [ ] Refine the published guide locally with descriptive media links, numbered defleshing disclosures, a subordinate `1.1.` troubleshooting mini-guide, OddArticulations-backed direct adipocere guidance, two case thumbnails, and two additional owner-photo links for the dark-skull and mummification scenarios. Remove mummified tissue from troubleshooting because it is a starting condition/above-ground outcome, not a maceration outcome. The refinement branch remains local until owner review; it does not authorize a push, PR, merge, deployment, or broader editorial scope.
...
- [x] Replace the single featured-skull entrance with a server-rendered ten-slot field and a route-local progressive enhancement for bounded visual-image parallax, focus/touch identity, and explicit arrangement changes.
- [x] Derive three deterministic arrangements from canonical published records so all 18 physical specimens appear across the cycle, with deliberate overlap between ten-slot windows and exact stable specimen links; keep only one bounded state rendered at a time.
- [x] Mix lateral-dominant canonical media with reviewed oblique, frontal, dorsal, ventral, and mandible views without constructing media filenames in page code.
...
The 2026-09-06 brief authorizes only the complete `/guides/skull-preparation` page and its coherent commits. The owner approved publication and a normal merge on 2026-09-08; the clean publication branch is `agent/skull-prep-release`. The 2026-09-10 refinement is a bounded follow-up on `preparation_guide_refinement`: it extends the same page/media contract with two additional owner-photo derivatives, square thumbnails, numbered method disclosures, internal detail subheadings, and the removal of the mummified-tissue troubleshooting case, without changing collection data or adding routes. System, content decisions, source/media contracts, acceptance checks and remaining limitations are maintained in [skull_preparation_guide.md](skull_preparation_guide.md). It does not authorize a Guides hub or other editorial pages. Exact local gate evidence and publication status live in `project_status.md`.
