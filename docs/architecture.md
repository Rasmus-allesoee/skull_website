...
Inside that island, every active view is projected into one 1,480 × 900 world from its reviewed image calibration and a shared pixels-per-millimetre factor. Deterministic arrangement helpers own initial positions and fitted-camera recovery; adding/removing a layer or reapplying an arrangement rebuilds all placements and fits the complete active set. Overlay mode groups every active layer by view token across all subjects. Layer translation and stacking, per-subject opacity, selected-layer state, and the field camera remain transient; only subjects, views, the directed pair, arrangement, and comparable-only mode enter the URL. Pointer movement writes layer transforms through animation frames and commits state on release, while keyboard arrows, explicit z-order/removal controls, zoom buttons/slider, `Fit all`, and `Reset layout` provide complete non-pointer recovery. Desktop wheel events pan the field; touch gesture classification leaves one-finger vertical page scrolling available and reserves two-finger movement for camera pan/pinch.
