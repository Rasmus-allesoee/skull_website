...
The owner-authorized `/guides/skull-preparation` route now prerenders the complete reviewed guide, illustrated workflow, semantic comparison tables, native disclosures, and a shared citation list from `.generated/preparation-guide-v1.json`. The restricted MDX compiler parses allowlisted literal Figure/Aside/Details blocks without executing JSX; strict frontmatter, stable anchor/citation/workflow integrity and public editorial-media validation run in the normal content build. Only the sticky contents drawer and reading-position state are client-enhanced; native disclosure/anchor access remains without JavaScript. Seven declared WebPs are processed explicitly from ignored masters; normal builds have no staging or network dependency. This implements ADRs 0001–0003 and does not modify collection CSVs or add MapLibre/search to the guide. See [skull_preparation_guide.md](skull_preparation_guide.md) for the bounded system and content decisions.
