...
`src/data/home.ts` composes the Home-only view model from those same published catalog records, reviewed map records, methodology reference, comparison records, and a validated Home-media manifest. Its deterministic round-robin projection distributes every published physical specimen across three six-record arrangements exactly once. The server renders arrangement one; a small route-local client island may advance the visit cursor after hydration or on explicit request without changing canonical identity or requesting all 18 images at once.
...
- Home's coordinate preview is a local static SVG projection of the same reviewed specimen coordinates. It has no basemap, provider request, controls, or independent geographic state and does not imply a live map.
...
The owner-supplied Preparation preview remains staging input at `agent_context/IMG_20251009_154046.jpg`. `pnpm media:process:home` creates only the cropped 1200×1600 sRGB WebP under `public/media/home/`; `content/home/home-media.json` owns its stable ID, public path, alt text, credit, and reserved-rights state. Normal content/media validation rejects a missing, undersized, non-WebP, or metadata-bearing derivative and emits a typed generated Home-media manifest.
