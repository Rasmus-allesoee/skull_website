...
`src/data/home.ts` composes the Home-only view model from those same published catalog records, their reviewed canonical media, and a validated Home-media manifest. Its deterministic projection builds three ten-slot arrangements from the 18 published physical specimens: arrangement windows overlap deliberately so every specimen appears across the cycle while only ten images render at once. Each placement carries one normalized depth value from the pure Home domain module; scale, opacity, blur, brightness, contrast, parallax travel, shadow, and base stacking are derived from that value rather than independently randomized. Lateral views remain dominant, with reviewed oblique, frontal, dorsal, ventral, and mandible views used for variety. The server renders arrangement one; a small route-local client island may advance the visit cursor after hydration or on explicit request without changing canonical identity or requesting the full media library at once.
...
The `/map/:path*` and generated `/maplibre/:path*` security headers permit only same-origin application assets, blob workers/images, and `https://tiles.openfreemap.org`; they do not add a wildcard analytics provider. Because CSP belongs to the current document, shared header/Home/specimen links enter `/map` through a native document navigation so the map policy is applied on first entry as well as reload. Production Web Analytics uses the already-allowed same-origin Vercel insights path, while map tiles remain the only route-specific external connection. Development adds `unsafe-eval` only for React/Turbopack diagnostics; the production header never includes it.
...
Comparison-reference sources use stable IDs under `content/references/`, are processed from ignored local staging through `pnpm media:process:reference`, and are committed only as reviewed WebP derivatives under `public/media/references/`. They pass the same sRGB, alpha, dimensions, subject-bounds, and metadata-stripping checks as specimen assets. Content compilation also derives the normalized alpha hit path for the reference manifest, so its comparison-field pointer target matches the visible silhouette like specimen layers. Exactly one reference is the default.

Owner-supplied Home thumbnail sources remain staging input under `agent_context/home_page/` (plus the separately retained Preparation source at `agent_context/IMG_20251009_154046.jpg`). `pnpm media:process:home` creates only the declared 1200 px-longest-edge sRGB WebPs under `public/media/home/`; `content/home/home-media.json` owns each stable ID, public path, alt text, credit, and reserved-rights state. Normal content/media validation rejects a missing, undersized, non-WebP, or metadata-bearing derivative and emits a typed generated Home-media manifest.
...
Inside that island, every active view is projected into one 1,480 × 900 world from its reviewed image calibration and a shared pixels-per-millimetre factor. Deterministic arrangement helpers own initial positions and fitted-camera recovery; ordinary arrangements rebuild all placements and fit the complete active set when layers change or a preset is reapplied. Overlay mode groups every active layer by view token across all subjects. The Custom arrangement centers active canvases once, then preserves existing manual placements/camera on layer changes while starting new layers at the world center. Layer translation and stacking, per-subject opacity, selected-layer state, and the field camera remain transient; only subjects, views, the directed pair, arrangement, and comparable-only mode enter the URL. Pointer movement writes layer transforms through animation frames and commits state on release, while keyboard arrows, explicit z-order/removal controls, zoom buttons/slider, `Fit all`, and `Reset layout` provide complete non-pointer recovery. Fitted-camera bounds use calibrated visible alpha subject rectangles, and the figure/image wrappers are pointer-transparent so only hit paths positioned within those rectangles can intercept selection. Ordinary desktop wheel/trackpad events remain document scrolling; a non-passive native listener consumes only modified wheel input for camera zoom and suppresses page scrolling/page zoom, while Cmd/Control-modified empty-space drag and two-finger touch provide camera navigation.

The subject rail is deliberately compact and height-bounded by the desktop workbench row. Subject cards and table headers share the same stable-ID reorder operation, while common/scientific names remain visible in analytical headers. Card and table reordering preserves the selected Difference pair's positional Skull slots and remaps its record IDs to the subjects now occupying those slots; subject removal retains the existing valid-ID fallback semantics. Card `dragstart` creates a temporary, exact-size clone in `document.body` and passes it to `DataTransfer.setDragImage`; `dragend` and unmount cleanup remove the clone, keeping the native preview limited to the card rather than the surrounding rail/workbench. The measurement section derives a primary profile-aware matrix plus a full union of selected profiles' primary/additional keys; cross-class width/height mappings are independent of the chosen Difference pair. Comparison images use the validated full public WebP directly so Next's thumbnail-sized optimizer response cannot become the limiting resolution at field zoom.

Workbench downloads stay client-side. The CSV serializer projects only the selected records' original numeric measurement keys and measured/approximate status into long or wide rows; it never serializes display-level mapped rows or computed Difference text. The PNG renderer is loaded only after an export request. It snapshots the visible viewport, camera, calibrated layer geometry, placements, stacking, opacity, label/scale-bar state, and existing public WebP paths before decoding images, then composites at up to three times CSS resolution within bounded canvas dimensions. The two backgrounds are pure black or a canvas reproduction of the UI grid/radial surface. Selection outlines, menus, and editing controls are outside the exported composition. Decoding/encoding errors surface as an inline status and do not download an incomplete image.
...
Server components own the museum shell, Home composition and hub previews, taxonomy landings, default catalog model/cards, static record content, methodology source loading, and route metadata. Client components are placed at the lowest practical interactive boundary. Home's only client island is the specimen field: server HTML already contains ten exact links, while enhancement owns bounded visual-image parallax, focus/touch identity, arrangement state, and the small-touch selection response; semantic links stay stationary during visual transforms, enhanced pointer/touch hit surfaces use alpha-derived paths so transparent regions do not block overlapping specimens, and a selected small-touch field applies only a bounded, distance-weighted repulsion to neighboring visual layers. One field-level identity card chooses an adjacent placement and clamps itself inside the field. The combined catalog island receives serialized published records and owns only discovery/view state; it keeps real links and prerendered default content. Native dialog controls own filters, measurement/age/condition guidance, and compact multi-specimen selection; the taxonomy drawer uses one responsive semantic nested-list component with a mobile focus trap; Home's comparison card is now a normal `/compare` link and does not embed workbench code. The measurement island receives a serialized validated reference model and adds hover/focus names, touch previews, synchronized selection, table-to-diagram targeting with originating-row restoration, and a compact non-modal detail panel over static figures and the complete semantic table. That panel tracks and inversely scales against the Visual Viewport during browser pinch/page-scale zoom, so it remains in the visible corner without a backdrop while the diagram stays usable. Accessibility state—names, expanded/selected/current semantics, live result/search changes, Escape, and focus restoration—is part of component APIs, not a post-release patch.

The owner-authorized `/guides/skull-preparation` route now prerenders the complete reviewed beginner guide, bounded illustrated workflow, semantic comparison tables, native method disclosures, inline citation popovers and a complete reference list from `.generated/preparation-guide-v1.json`. The restricted MDX compiler parses allowlisted literal Figure/Aside/Details/Subheading/Disclosure blocks and descriptive media links without executing JSX; strict frontmatter, stable heading/disclosure/citation/workflow integrity and public editorial-media validation run in the normal content build. The sticky contents drawer and reading-position state also open hash-targeted native disclosures; the summaries, anchors, full reference list and all content remain available without JavaScript. Eighteen declared WebPs are processed explicitly from ignored masters; ordinary image links and collapsed troubleshooting case thumbnails progressively enhance to native quick-preview/body-level lightboxes, while direct WebP destinations remain available without JavaScript. Normal builds have no staging or network dependency. This implements ADRs 0001–0003 and does not modify collection CSVs or add MapLibre/search to the guide. See [skull_preparation_guide.md](skull_preparation_guide.md) for the bounded system and content decisions.
