...
- Final local verification: `CI=true pnpm check` passed formatting, lint, media/content validation, strict TypeScript, 81/81 unit/component tests, and six expected invalid-fixture failures. Next.js 16.3.4 built all 77/77 static routes. Chromium passed 79/79 browser journeys without retry. The Firefox/WebKit matrix passed 153 journeys with four intentional Chromium-protocol skips; its single Firefox popup-wheel timing flake was corrected and then passed six consecutive Firefox/WebKit runs with retries disabled.
- Manual production-render inspection covered Home at 1440×900 and 390×844, the mobile Species catalog, and the mobile Map menu layered above the map/control surface. No horizontal overflow or menu obstruction was observed. Owner-supplied untracked `agent_context` files and ignored Lighthouse/browser artifacts remain outside the release checkpoint.
- Final owner review found three release blockers, now corrected locally. Schema version 6 replaces the mistakenly public `distinguishing_features` field with a dedicated blank-by-default `condition_description`; all personal distinguishing notes were scrubbed from canonical data, the migration declaration, generated output, and public UI. True-scale comparison now admits every published specimen with measured/approximate maximum length and an oriented lateral image, so all 18 current specimens—including secondary badger and seal records—are eligible. The Map fallback-class filter now uses one valid MapLibre expression dialect, and recoverable pre-load provider events no longer dismiss a map that can still finish loading.
- Focused verification for those corrections: the Phase 6 audit still accounts for 22 taxa, 52 specimens, and 104 PNGs; `migration:prepare` reports zero canonical differences; the complete `pnpm check` passes with 82/82 tests and six expected invalid-fixture failures; and the production build prerenders 77/77 routes. A development-browser reproduction reaches `data-map-ready=true`, exposes two accessible clusters, shows no fallback, and logs no errors. Four affected production-browser journeys pass without retry, covering normal Map readiness, WebGL/provider failure, condition rendering, and comparison on a newly measured secondary specimen.

- The first Vercel Preview attempt reached the build but Vercel's Node 24.19.0 runtime rejected the former exact `24.18.0` engine constraint. The package now accepts `>=24.18.0 <25`, retaining 24.18.0 for local validation while allowing Vercel's current Node 24.x patch release. `.vercelignore` excludes ignored owner/staging/build output from CLI uploads; the first dry-run now sends 358 files / approximately 31 MB instead of the local 1.6 GB working tree.

### 0.2 Map provider error handling correction (2026-09-08)

- Root cause: the release-hardening change removed MapLibre's `error` listener. MapLibre therefore reported an expected aborted style request from development Strict Mode teardown, or a transient provider failure, as an unhandled `AJAXError` in the Next.js development overlay. The current OpenFreeMap Fiord endpoint and its dependent tile, sprite, and font endpoints respond successfully; the collection data and map projection were not the cause.
- Fixed locally by binding a lifecycle-aware MapLibre error listener, ignoring stale/disposed map instances, and retaining the existing delayed user-facing fallback only when the active map never loads. The semantic specimen list and exact links remain available in a genuine provider outage.
- Verification: `CI=true corepack pnpm build` prerendered 77/77 routes; `PLAYWRIGHT_PORT=3102 corepack pnpm test:e2e tests/e2e/map.spec.ts` passed 23/23 journeys including the new no-`AJAXError` provider-failure assertion; and `CI=true corepack pnpm check` passed formatting, lint, media/content validation, TypeScript, 82/82 unit/component tests, and six expected invalid-fixture failures. This correction is local only; no push, PR, merge, or deployment was performed.
...
- Kept table-origin selection context through dismissal: clearing a selection activated from the definitions table returns the view to the table and restores the table action focus without changing diagram-origin behavior.
- Removed source-only parentheticals from the canonical descriptions (including the five feedback-listed labels and the matching #8 note); no formal citation records were invented.
...
- Hardened touch activation for mobile MapLibre layer taps with a movement threshold and synthesized-click suppression. Mobile popup cards now stay fixed within the map surface while the map is panned, preventing the card from jumping with the marker; desktop popup tracking is unchanged. The development map CSP permits React/Turbopack's required `unsafe-eval` diagnostics only in development, removing the misleading issue badge without weakening production CSP. The approximate-location key now shows only its neutral outer ring.
- Kept narrow approximate-location popup cards within their bounded height by placing Precision and Uncertainty radius side-by-side while retaining full-width Location and Date rows; exact-location cards keep their existing layout.
- The production build generated 76 static routes. The focused map suite's two timing-sensitive camera/popup assertions were rerun successfully after stopping in-flight camera easing; the other 20 journeys passed in the same run. Direct production mobile checks opened both individual and cluster popups by touch, kept the popup anchor fixed during a touch pan, and confirmed a clean production console. The Turbopack mobile check reported zero errors/warnings and no issue badge. Final TypeScript, formatting, and lint checks passed. No push, pull request, merge, or remote CI action is included.
...
- Added owner to the Collection record; changed `SPEC-0001` from the overly broad `damaged` state to `good`; retained `adult` without exposing unsupported `legacy stage 4` wording. The later schema-version-6 release correction removes private distinguishing notes from public data entirely.
...
| Focused browser/accessibility | `CI=true PLAYWRIGHT_PORT=3106 pnpm test:e2e tests/e2e/methodology.spec.ts`: 6/6 Chromium journeys cover canonical counts/media, corrected layout/marker/ring behavior, hover/focus/keyboard non-modal details, movement-aware outside clearing, repeated highlighting, table-to-diagram isolation and return, compact tappable mobile preview placement/details, 360/390 px containment, effective 200% reflow, emulated 200% visual-viewport anchoring, forced colors, reduced motion, static/no-JavaScript content, focus restoration, zero console errors, and zero axe violations | Pass |
| Full browser regression | `CI=true PLAYWRIGHT_PORT=3107 pnpm test:e2e` completed with 61/61 Chromium journeys, including all six methodology journeys, in 2.5 minutes. No existing map flake reproduced and no map source/test file changed in this refinement | Pass |
| Manual visual/responsive | Production Playwright inspection covers the revised dorsal/ventral lead at 1440 × 900, cropped lateral geometry, the paired mandible/canine row, compact fixed details, compact tappable 390 px touch previews/details, and 200% visual-viewport behavior. Anatomy and annotations remain registered and uncropped, arrowheads/numbers remain legible, mobile and desktop have no horizontal page overflow, and the browser console has zero errors | Pass |
| Git/base/scope audit | Branch `agent/next-additions` starts from the post-merge Phase 5 base. The intended refinement diff is limited to canonical methodology definitions/geometry, route/island/styles, focused tests, and matching canonical documentation; supplied raw/annotated staging, review screenshots, and unrelated owner files remain unstaged/ignored | Pass |
...
### 2026-08-31 — Measurements feedback v2 refinement verified locally

- The owner supplied `agent_context/prompt_measurement_page_feedback_v2.md`. The bounded refinement adds movement-aware primary-tap dismissal (swipes, pointer cancellation, and multi-touch preserve state), adaptive diagram/visual-viewport tooltip placement on desktop and mobile, table-origin return-to-table behavior, source-only parenthetical cleanup, dorsal/ventral crop tightening, dorsal #12 ring clearance, and the approximately 820 px layout breakpoint.
- Source verification, `CI=true pnpm check`, the 77-output production build, focused 6/6 Chromium/axe/no-JavaScript/360–390 px/visual-zoom suite, full 61/61 Chromium regression, and manual production visual checks pass. The refinement remains local for renewed owner review; no push, pull request, merge, remote CI, or next phase is authorized.

### 2026-08-31 — Measurements feedback v3 refinement verified locally
...
### 2026-09-02 — Home hero hitbox/mobile popup refinement verified locally
