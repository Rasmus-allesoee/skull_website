...
- The first ten-specimen arrangement is server-rendered and remains fully useful without JavaScript. Enhancement adds subtle bounded pointer/touch depth to the visual image while keeping each link hitbox fixed, keyboard-equivalent focus disclosure, first-tap touch selection, a deliberate second-tap/direct identity action, and a secondary next-arrangement control. Lateral views remain dominant with reviewed alternate angles mixed in. Reduced motion removes depth transforms and nonessential transitions.
