...
- One compact exploration hub contains live Species, Map, Compare, Measurements, and Preparation whole-card links. The Compare card opens the calibrated standalone workbench without loading that workbench into Home.
...
The Phase 2.2 specimen-page scale card established the reusable calibrated comparison primitives now composed by the approved post-v1 `/compare` workbench. The workbench supports up to five selected subjects and ten calibrated view layers while retaining one explicit directed measurement-difference pair. The owner later authorized analysis-ready CSV export of selected source measurements and a field-only PNG export. Split sliders, 360°, 3D, illustrations, direct contributions, database administration, custom-event analytics, localization, and broader data export remain deferred work and require their documented prerequisites and a new scope decision.
