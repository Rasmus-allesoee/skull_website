...
- A concise introduction and primary `/species` action sit beside a curated 2D specimen field. Its three bounded arrangements cover all 18 published physical specimens without eagerly rendering the whole library; every visible skull has an accessible identity and exact nested specimen link.
- The first six-specimen arrangement is server-rendered and remains fully useful without JavaScript. Enhancement adds subtle bounded pointer depth, keyboard-equivalent focus disclosure, first-tap touch selection, a deliberate second-tap/direct identity action, and a secondary next-arrangement control. Reduced motion removes depth transforms and nonessential transitions.
...
- The Map card is a deterministic static plot of reviewed canonical coordinates. Home never loads MapLibre, provider tiles, map controls, or the map result rail.
