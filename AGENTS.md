...
- True-to-scale comparison uses explicit reviewed per-image calibration spans tied to canonical measurements. Lateral orientation remains explicit; dorsal/ventral use reviewed maximum-length spans, frontal uses the profile-appropriate width, mandible dorsal uses a diagonal hemimandible landmark span, and oblique remains ineligible. Approximate reference measurements must remain labelled as approximate.
- The static `/compare` route composes one client workbench island over the shared comparison domain: one to five subjects, at most ten calibrated view layers, one shared physical-scale world/camera, transient free placement/stacking/opacity, one directed class-aware difference pair, URL-backed analytical state, and a semantic responsive table. Client-side CSV downloads expose only selected subjects' original recorded measurement values and statuses; a high-resolution PNG captures the current visible field with either pure black or the UI backdrop. It does not load MapLibre, Orama, a runtime database, or external data.
- The schema-version-7 model stays unified in `specimens.csv`: mammal, bird, and fallback profiles control applicability and presentation without parallel specimen tables. The validated `species_name + specimen_id_raw` pair is a curator crosswalk only; immutable `SPEC-####` remains the public identity. Public condition evidence belongs only in `condition_description`; private `distinguishing_features` notes never enter canonical or generated public data. Class-specific fields require explicit `not_applicable` status outside their profile.
...
- Preparation media links use the typed editorial-media contract, a native portal lightbox as progressive enhancement, and a direct WebP link as the no-JavaScript fallback. Troubleshooting cases remain native disclosures with their details collapsed by default.
...
- `agent_context/home_page/prompt_hero_field_refinement.md` — owner feedback defining the bounded Home hero depth/composition and identity-card refinement.
- `agent_context/home_page/hero_specimen_labels_error.png` — owner visual evidence for hero identity-card clipping; context only, never a runtime asset.
...
- `agent_context/preparation_page/` — ignored owner preparation brief/photographs, the feedback-supplied oily-patch, brain-material, adipocere, dark-skull, and mummified-badger sources, and generated degreasing master; maintenance-only input. The canonical page system is `docs/skull_preparation_guide.md`; reviewed content/media live under `content/guides/`, and `pnpm media:process:preparation` creates the validated public derivatives.
- `agent_context/prompt_prep_guide_page_feedback_v1.md`, `troubleshooting_adipocere_feedback.md`, `reddit_processing_carcass_101.md`, `animal_bone_preparation_guide.pdf`, and `JoNSC-Vol7-Munoz-Saba_et_al_2020_Dermestidae_cleaning.pdf` — owner feedback and ignored research inputs for the bounded preparation-guide v1 overhaul and its local refinement; canonical decisions and reviewed citations live in `docs/` and `content/guides/`. The refinement also records the reviewed OddArticulations maceration and adipocere pages as external sources.
