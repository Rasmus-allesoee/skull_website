...
- Home server-renders one complete six-specimen field state and progressively enhances only that field. Three bounded arrangements cover all 18 published physical specimens exactly once per cycle; each image retains its canonical `MediaAsset` identity and exact specimen route. Home never loads MapLibre or the catalog search index.
