...
Current phase: **v1.1.0 release integration is underway for the standalone skull-comparison workbench and the owner-approved preparation-guide refinement.** Production remains at v1.0.1 at `https://skullwebsite-xi.vercel.app`. Preparation PR #24 and comparison PR #25 are open against `main`; the preparation checks have passed and comparison checks are running. The comparison branch will be refreshed from `main` after the preparation refinement lands. The previously deferred standalone comparison route is now in this release scope. Consult `docs/project_status.md` for exact evidence, dependency-PR dispositions, and the next action.
